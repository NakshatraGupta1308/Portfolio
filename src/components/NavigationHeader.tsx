...
export const NavigationHeader: React.FC<NavigationHeaderProps> = ({ onToggleQnA }) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-[#090a0c]/90 backdrop-blur-md border-b border-[rgba(255,255,255,0.12)]">
      <div className="h-16 w-full px-3 sm:px-5 lg:px-6 xl:px-8 flex items-center justify-between gap-2 sm:gap-4 lg:gap-6">
        {/* Logo & Identity */}
        <div className="flex items-center gap-2.5 shrink-0">
          <span className="w-2 h-2 rounded-full bg-[#00ff66] ring-2 ring-[#00ff66]/20 shrink-0 animate-pulse"></span>
          <Link
            to="/"
            className="flex items-center gap-1.5 font-mono text-[12px] tracking-wider text-[#ffffff] uppercase hover:text-[#ffb3af] transition-colors whitespace-nowrap"
          >
            <span className="font-semibold">NAKSHATRA GUPTA</span>
            <span className="text-[#52545d]">//</span>
            <span className="text-[#8d9099] hidden lg:inline">CprE @ Iowa State</span>
          </Link>
        </div>

        {/* Desktop Navigation Links */}
        <nav className="hidden md:flex items-center gap-3 lg:gap-5 xl:gap-7 h-full shrink-0">
          <Link
            to="/"
            className={`font-mono text-[12px] uppercase tracking-wider whitespace-nowrap h-full flex items-center transition-colors border-b-2 px-1 ${
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2 py-0.5 bg-[#121316] border border-[rgba(255,255,255,0.1)] hover:border-[#ff2a3b]/50 text-[#8d9099] hover:text-[#ffffff] text-[10px] font-mono tracking-wider transition-all cursor-pointer whitespace-nowrap shrink-0"
          ...
          >
            <span className={`material-symbols-outlined text-[13px] ${sfxMuted ? 'text-[#52545d]' : 'text-[#ff2a3b]'}`}>
          ...
          {onToggleQnA && (
            <button
              onClick={onToggleQnA}
              className="hidden lg:flex items-center gap-1.5 px-2 py-0.5 bg-[#121316] border border-[#ff2a3b]/40 text-[#ff2a3b] hover:bg-[#ff2a3b] hover:text-[#ffffff] text-[10px] font-mono tracking-wider transition-all cursor-pointer whitespace-nowrap shrink-0"
          ...
          {/* Availability Status Pill - Responsive to prevent screen overflow */}
          <div className="hidden md:flex items-center gap-1.5 px-2.5 py-0.5 bg-[#1f2022] border border-[rgba(255,42,59,0.4)] rounded-full whitespace-nowrap shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-[#ff2a3b] animate-pulse shrink-0"></span>
            <span className="font-mono text-[10px] tracking-widest text-[#ffffff] uppercase font-semibold">
              <span className="hidden 2xl:inline">OPEN FOR FALL 2026 &amp; BEYOND</span>
              <span className="2xl:hidden">OPEN: FALL 2026+</span>
            </span>
          </div>
