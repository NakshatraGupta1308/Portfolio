  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setErrorMsg(data.error || 'Failed to dispatch inquiry. Please check your connection.');
      }
    } catch (err) {
      setErrorMsg('Could not transmit dispatch to backend server. Please email ng1308@iastate.edu directly.');
    } finally {
      setLoading(false);
    }
  };
      ...
      {receipt ? (
        <div className="p-6 bg-[#090a0c] border border-[#00ff66] space-y-3 font-mono text-xs">
          <div className="flex items-center gap-2 text-[#00ff66] font-bold text-sm">
            ...
            <span>DISPATCH CONFIRMED &amp; LOGGED</span>
          </div>
          ...
          <button
            ...
            className="mt-2 px-4 py-2 bg-[#ffffff] text-[#090a0c] font-bold text-xs uppercase hover:bg-[#ff2a3b] hover:text-[#ffffff] transition-colors"
          >
